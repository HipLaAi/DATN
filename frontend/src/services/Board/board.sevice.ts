...
export const updateIBoardAPI = async (id: any, data: any): Promise<any> => {
    const res = await apiServer?.post(`/api/board/updateinformation/` + id, data);
    return res?.data;
};

export const updateBackgroundBoardAPI = async (id: any, data: any): Promise<any> => {
    const res = await apiServer?.post(`/api/board/updatebackground/` + id, data);
    return res?.data;
};
